...
当需要在真实场景或特定数据集上使用模型时，建议进行微调：

1. **真实场景应用**：模型在合成数据上训练，真实场景效果可能不佳
2. **特定领域适配**：如特定类型的文档、表格等
3. **质量提升**：针对特定数据集进一步优化效果

### 微调步骤

1. **准备微调数据**：
   ```bash
   # 步骤1: 使用数据增强脚本生成带涂抹痕迹的训练图像
   # 确保 tran_add_up/ 目录结构正确，然后运行：
   python augment_images.py

   # 步骤2: 准备数据集
   mkdir -p custom_train_dataset/images
   mkdir -p custom_train_dataset/gts
   # 将生成的增强图像和原始gts复制到新目录
   cp -r tran_add_up/augmented_images/* custom_train_dataset/images/
   cp -r tran_add_up/gts/* custom_train_dataset/gts/
   
   # 步骤3: 生成mask
   python compute_mask.py  # 确保脚本中的路径指向 custom_train_dataset
   ```
...
2. **微调命令**：
   ...
   CUDA_VISIBLE_DEVICES=0 python3 train.py \
       --batchSize 4 \
       --dataRoot './custom_dataset/images' \
       --net 'idr' \
       --lr 5e-5 \                    # 使用较小的学习率
       --pretrained 'STE_idr_best.pdparams' \  # 加载预训练模型
...
**Q10: 数据增强如何调整？**
A10: 可以修改`data/dataloader.py`中的增强策略：
- 调整翻转概率
- 修改旋转角度范围
- 添加其他增强方法
