...
当需要在特定数据集上获得最佳性能时，我们推荐使用一种更高级的微调策略。该策略使用一个脚本来自动化数据集的准备，包括创建验证集，并使用一个专门的训练脚本，以一定概率混合使用原始数据和增强数据。

### 微调优势
- **自动化**: 使用 `prepare_finetune_dataset.py` 脚本一键完成数据清洗、校验和验证集划分。
- **鲁棒性更强**: 通过按比例混合原始图像和增强图像，模型可以同时学习两种数据分布，提高泛化能力。
...
使用专门的 `train_finetune.py` 脚本开始训练。它支持混合数据加载和周期性验证。
...
- `--batchSize`: 根据你的GPU显存大小调整，如果遇到显存不足（CUDA out of memory），请减小此值。
